	...
	moduleSpecifics := shared.ModuleSpecifics{
		...
		VerificationFunc: func(verificationId string, bskyHandle string) (bool, error) {
			...
			if err != nil {
				...
				return false, fmt.Errorf("Error fetching the MVP profile, probably caused by an invalid MVP ID: "+err.Error())
			}
			...
			if err != nil {
				...
				return false, fmt.Errorf("Error decoding MVP JSON, probably caused by an invalid MVP ID: "+err.Error())
			}
